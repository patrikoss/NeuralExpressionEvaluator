...
def partition_data_in_folder(root_dataset_folder, level1folder, level2folder):
    """
    Partitions the data found in the given folder in the 'all' level into train,test,dev(and train_debug)
    versions
    """
    ...
    for filename in os.listdir(partition_folder):
        ...
        for partition_type in level1[1:]:
            save_path = pjoin(root_dataset_folder, partition_type, "raw_npz_symbols", filename)
            pd_X, pd_y = partitioned_data[partition_type]
            save_dataset(pd_X, pd_y, save_path)

if __name__=='__main__':
    images_root_folder = "/home/patryk/PycharmProjects/MathExpressionEvaluator/dataset2/all/raw_symbols/jpg_symbols"
    dataset_root_folder="/home/patryk/PycharmProjects/MathExpressionEvaluator/dataset"
